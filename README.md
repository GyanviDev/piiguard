...
2. **Differential Privacy** — Numeric values (financial figures, counts) have Laplace-distributed noise applied: `noisy_value = real_value + Laplace(0, Δf/ε)` with `ε=0.1`
...
This system implements **(ε,δ)-Differential Privacy** via the **Laplace Mechanism**:

> A randomised algorithm M is ε-differentially private if for any two adjacent datasets D and D', and any output S: **P[M(D)∈S] ≤ e^ε × P[M(D')∈S]**

With `ε=0.1` and sensitivity `Δf=1`, noise is drawn from `Laplace(0, 10)` — providing strong privacy guarantees for financial and medical data.
...
cd piiguard
# Add your Groq API key to src/main/resources/application.properties
